...
// Check if intptr_t is not already defined
#ifndef intptr_t
    #include <stdint.h>
#endif
...
**     lstrcpynA(ini_path,(char*)SendMessage(hwnd_winamp,WM_WA_IPC,0,IPC_GETINIFILE),sizeof(ini_path)/sizeof(char));
**   }
**   else{
**   char* p = ini_path;
...
**     // this gets the string of the full ini file path
**     lstrcpynW(ini_path,(wchar_t*)SendMessage(hwnd_winamp,WM_WA_IPC,0,IPC_GETINIFILEW),sizeof(ini_path)/sizeof(wchar_t));
**   }
**   else{
**   wchar_t* p = ini_path;
...
/* (requires Winamp 5.66+)
** wchar_t *ml_ini=(wchar_t*)SendMessage(hwnd_winamp,WM_WA_IPC,0,IPC_GETMLINIFILEW);
...
/* (requires Winamp 5.53+)
** SendMessage(hwnd_winamp,WM_WA_IPC,(WPARAM)(HWND)parent,IPC_UPDATEDIALOGBOXPARENT);
...
/* (requires Winamp 5.66+)
** SendMessage(hwnd_winamp,WM_WA_IPC,(WPARAM)mode,IPC_SET_JTF_LOAD_MODE);
...
** int mode = SendMessage(hwnd_winamp,WM_WA_IPC,-666,IPC_SET_JTF_LOAD_MODE);
*/

#define IPC_GET_JTF_LOAD_MODE(hwnd_winamp) SendMessage(hwnd_winamp,WM_WA_IPC,(WPARAM)-666,IPC_SET_JTF_LOAD_MODE)
/* (requires Winamp 5.66+)
** int curpan = IPC_GETPANNING(hwnd_winamp);
...
/* (requires Winamp 5.0+ unless otherwise specified)
**
** pass 0 for a copy of the skin HBITMAP (use DeleteObject(..) on this when done or requesting a new copy)
** pass 1 for name of font to use for playlist editor likeness (char* version)
...
typedef struct embedWindowState
{
  ...
  // WACUP set this bit to allow WACUP to properly handle double sizing of the window
  //       instead of just leaving it like Winamp where it just looks off
...
/* (requires Winamp 5.66+)
** int done = SendMessage(hwnd_winamp,WM_WA_IPC,(WPARAM)(embedWindowState*)ws,IPC_EMBED_ADD_LEGACY);
...
/* (requires Winamp 5.66+)
** int done = SendMessage(hwnd_winamp,WM_WA_IPC,(WPARAM)(embedWindowState*)ws,IPC_EMBED_REMOVE_LEGACY);
** You pass an embedWindowState* filled with any appropriate details and ensure you add
** EMBED_FLAGS_LEGACY_WND to the 'flags' member to ensure it will be handled correctly.
** Returns 1 if the request was able to be processed.
*/
...
/* (requires Winamp 5.66+)
** int done = SendMessage(hwnd_winamp,WM_WA_IPC,(WPARAM)(embedWindowState*)ws,IPC_EMBED_UPDATE_LEGACY_POS);
...
**   *(void**)&SnapToScreen = (void*)SendMessage(hwnd_winamp,WM_WA_IPC,0,IPC_GET_EMBED_SNAPFUNC);
**   SnapToScreen(&r);
**
** or
**
...
**   *(void**)&SnapWindowToAllWindows = (void*)SendMessage(hwnd_winamp,WM_WA_IPC,1,IPC_GET_EMBED_SNAPFUNC);
...
** int has_video_support=SendMessage(hwnd_winamp,WM_WA_IPC,0,IPC_HAS_VIDEO_SUPPORT);
...
/*
** These are defined control ids for using some of the shared resources from
** winamp.exe for tabbed preference dialogs and for plug-in preference lists
*/

#define IDD_TABBED_PREFS_DIALOG         257
#define IDC_TABBED_PREFS_TAB            1279
